...
document.addEventListener("DOMContentLoaded", () => {
  ...
  async function displayData() {
    ...
    try {
      const response = await axios.post("http://localhost:3000/node-api/get-user-icon-data/"+username);
  ...
  async function updateData(id, newData) {
    try {
      const response = await axios.put(`http://localhost:3000/node-api/update-user-icon-data/${id}`, newData);
  ...
  async function deleteData(id) {
    try {
      const response = await axios.delete(`http://localhost:3000/node-api/delete-user-icon-data/${id}`);
